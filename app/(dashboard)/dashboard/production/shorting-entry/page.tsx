...
import { ShortingEntry } from "@/types/production";
...
export default function ShortingEntryPage() {
  const [shortingEntries, setShortingEntries] = useState<ShortingEntry[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [editingEntry, setEditingEntry] = useState<ShortingEntry | null>(null);
  const [viewingEntry, setViewingEntry] = useState<ShortingEntry | null>(null);
  ...
  const downloadShortingEntriesCsv = async () => {
    try {
      ...
      const csvRows = [
        headers.join(","),
        ...(result.data || []).map((entry: ShortingEntry) =>
          [
            entry.entry_no || "",
            entry.entry_date || "",
  ...
  const getQualityRate = (entry: ShortingEntry) => {
  ...
  const getQualityBadgeColor = (rate: string) => {
