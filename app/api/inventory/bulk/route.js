  ...
  try {
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      if (!validClassifications.includes(record.classification)) {
        errors.push(
          `Row ${rowNumber}: Invalid classification "${
      ...
      if (isNaN(pricePerPiece) || pricePerPiece < 0) {
        errors.push(
          `Row ${rowNumber}: Invalid price_per_piece "${record.price_per_piece}"`
        );
        continue;
      }
      ...
      const inventoryData = {
        challan_no:
          record.challan_no || `INV-${record.product_sku}-${Date.now()}-${i}`,
    ...
    if (operation === "update") {
      ...
      for (const record of validatedRecords) {
        try {
          ...
          if (error) {
            updateErrors.push(
              `Failed to update ${record.product_sku}: ${error.message}`
            );
          } else {
            updatedCount++;
          }
          ...
          updateErrors.push(
            `Failed to update ${record.product_sku}: Unexpected error`
          );
        }
      }
  ...
  try {
    ...
    return new NextResponse(csvContent, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="inventory-export-${
          classification || "all"
