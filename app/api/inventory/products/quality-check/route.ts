...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!qcStatus || !["pending", "in_progress", "passed", "failed", "requires_rework"].includes(qcStatus)) {
      return NextResponse.json(
    ...
    for (const product of products) {
      try {
        ...
        if (updateError) {
          errors.push(`Product ${product.product_sku}: Failed to update QC status - ${updateError.message}`);
          continue;
        }
        ...
        console.error(`Error updating QC for product ${product.product_sku}:`, error);
        errors.push(`Product ${product.product_sku}: Unexpected error during QC update`);
      }
    }
  ...
  try {
    ...
    const { data: stats } = await supabase
      ...
      .then(({ data }) => {
        ...
        data?.forEach(item => {
          const status = item.product_status as keyof typeof stats;
