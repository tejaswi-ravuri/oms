...
export async function GET(request) {
  try {
    ...
    if (search) {
      query = query.or(
        `product_name.ilike.%${search}%,product_sku.ilike.%${search}%,product_description.ilike.%${search}%`
      );
    }
    ...
    const uniqueCategories = [
      ...new Set(categories?.map((c) => c.product_category) || []),
    ];
    const uniqueMaterials = [
      ...new Set(materials?.map((m) => m.product_material) || []),
    ];
    const uniqueColors = [
      ...new Set(colors?.map((c) => c.product_color) || []),
    ];
...
export async function POST(request) {
  try {
    ...
    if (body.product_description !== undefined)
      productData.product_description = body.product_description;
    if (body.product_qty !== undefined)
      productData.product_qty = body.product_qty;
    if (body.product_image !== undefined)
      productData.product_image = body.product_image;
    if (body.product_status !== undefined)
      productData.product_status = body.product_status;
    if (body.product_material !== undefined)
      productData.product_material = body.product_material;
    if (body.product_color !== undefined)
      productData.product_color = body.product_color;
    if (body.product_size !== undefined)
      productData.product_size = body.product_size;
