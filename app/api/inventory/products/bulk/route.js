...
export async function POST(request) {
  try {
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      if (existingProduct) {
        errors.push(
          `Row ${rowNumber}: Product with SKU "${record.product_sku}" already exists`
        );
        continue;
      }
      ...
      const productData = {
        ...
        is_refurbished:
          record.is_refurbished === "true" || record.is_refurbished === "TRUE",
        original_manufacturing_cost:
          parseFloat(record.original_manufacturing_cost || "0") || 0,
        created_by: user.id,
