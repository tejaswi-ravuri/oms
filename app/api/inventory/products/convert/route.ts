  ...
  try {
    ...
    for (const product of products) {
      try {
        ...
        if (inventoryError) {
          errors.push(`Product ${product.product_sku}: Failed to create inventory entry - ${inventoryError.message}`);
          continue;
        }
        ...
        const { error: updateError } = await supabase
          ...
          .update({
            ...
            product_description: product.product_description
              ? `${product.product_description}\n\nConverted to inventory on ${new Date().toISOString().split('T')[0]} (INV-${inventoryItem.id})`
              : `Converted to inventory on ${new Date().toISOString().split('T')[0]} (INV-${inventoryItem.id})`,
          })
          .eq("id", product.id);
        
        if (updateError) {
          errors.push(`Product ${product.product_sku}: Failed to update product with conversion info - ${updateError.message}`);
          // Rollback inventory creation
          await supabase.from("stitching_challans").delete().eq("id", inventoryItem.id);
          continue;
        }
        ...
        console.error(`Error converting product ${product.product_sku}:`, error);
        errors.push(`Product ${product.product_sku}: Unexpected error during conversion`);
      }
    }
