...
export async function GET(request) {
  try {
    ...
    let query = supabase.from("stitching_challans").select(`
        id,
        challan_no,
        challan_date,
        product_name,
    ...
    const skus = [
      ...new Set(items?.map((item) => item.product_sku).filter(Boolean)),
    ];
    ...
    if (skus.length > 0) {
      const { data: products } = await supabase
        .from("products")
        .select(
          "product_sku, product_name, product_category, manufacturing_cost"
        )
        .in("product_sku", skus);
    ...
    const transformedItems =
      items?.map((item) => {
        ...
        const productInfo = item.product_sku
          ? productsMap.get(item.product_sku)
          : null;

        // Determine primary classification based on totals
        let classification = "unclassified"; // removed TS type
        ...
        const pricePerPiece =
          item.rate_per_piece || productInfo?.manufacturing_cost || 0;
        ...
        return {
          ...
          quantity: quantity,
          product_name:
            productInfo?.product_name || item.product_name || "Unknown Product",
    ...
    const uniqueQualities = [
      ...new Set(statusData?.map((s) => s.status) || []),
    ].sort();

    // Calculate summary statistics
    const summary = {
      total_items: transformedItems.length,
      total_good: transformedItems.reduce(
        (sum, item) => sum + (item.total_good || 0),
        0
      ),
      total_bad: transformedItems.reduce(
        (sum, item) => sum + (item.total_bad || 0),
        0
      ),
      total_wastage: transformedItems.reduce(
        (sum, item) => sum + (item.total_wastage || 0),
        0
      ),
      total_cost: transformedItems.reduce(
        (sum, item) => sum + item.total_cost,
        0
      ),
      good_items: transformedItems.filter((i) => i.classification === "good")
        .length,
      bad_items: transformedItems.filter((i) => i.classification === "bad")
        .length,
      wastage_items: transformedItems.filter(
        (i) => i.classification === "wastage"
      ).length,
    };
