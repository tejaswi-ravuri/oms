...
export async function POST(request: NextRequest) {
  try {
    ...
    if (missingHeaders.length > 0) {
      return NextResponse.json(
        { error: `Missing required headers: ${missingHeaders.join(", ")}. Found headers: ${headers.join(", ")}` },
        { status: 400 }
      );
    }
    ...
    for (let i = 1; i < lines.length; i++) {
      try {
        ...
        if (existingLedger) {
          console.log(`⚠️ Row ${i + 1}: Business name "${rowData.business_name}" already exists - skipping`);
          errors.push(`Row ${i + 1}: Business name "${rowData.business_name}" already exists - skipping`);
          continue;
        } else {
          console.log(`✅ Row ${i + 1}: Business name "${rowData.business_name}" is unique - proceeding`);
        }
        ...
        if (rowData.gst_number && rowData.gst_number.trim()) {
          ...
          if (gstNumber && !/^[0-9A-Z]{15}$/.test(gstNumber)) {
            console.log(`❌ Row ${i + 1}: Invalid GST number format: ${gstNumber}`);
            errors.push(`Row ${i + 1}: Invalid GST number format: ${gstNumber}. Must be 15 alphanumeric characters`);
            continue;
          }
        }
        ...
        if (rowData.pan_number && rowData.pan_number.trim()) {
          ...
          if (panNumber && !/^[0-9A-Z]{10}$/.test(panNumber)) {
            console.log(`❌ Row ${i + 1}: Invalid PAN number format: ${panNumber}`);
            errors.push(`Row ${i + 1}: Invalid PAN number format: ${panNumber}. Must be 10 alphanumeric characters`);
            continue;
          }
        }

        console.log(`✅ Row ${i + 1}: All validations passed - preparing to insert`);

        // Generate unique ledger_id
        const ledger_id = `LDG-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`;
        ...
        const insertData = {
          ...
          gst_number: rowData.gst_number?.replace(/\s/g, "").toUpperCase() || null,
          pan_number: rowData.pan_number?.replace(/\s/g, "").toUpperCase() || null,
        ...
        insertErrors.push(`Row ${i + 1}: Failed to process row - ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
    ...
    return NextResponse.json({
      message: `Successfully imported ${insertedLedgers.length} of ${lines.length - 1} ledgers`,
      imported: insertedLedgers.length,
      total: lines.length - 1,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: ledgers, error } = await query.order("created_at", { ascending: true });
    ...
    return new NextResponse(csvContent, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="ledgers_export_${new Date().toISOString().split('T')[0]}.csv"`
      }
    });
