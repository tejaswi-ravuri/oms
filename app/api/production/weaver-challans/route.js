...
export async function GET(request) {
  try {
    ...
    if (search) {
      query = query.or(
        `challan_no.ilike.%${search}%,material_type.ilike.%${search}%,batch_number.ilike.%${search}%`
      );
    }
  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Missing required fields: challan_no, challan_date, weaver_ledger_id, material_type, quantity_sent_meters",
        },
        { status: 400 }
      );
    }
    ...
    const cleanedData = {
      ...
      quantity_received_meters: body.quantity_received_meters
        ? parseFloat(String(body.quantity_received_meters))
        : 0,
      rate_per_meter: body.rate_per_meter
      ...
      total_grey_mtr: body.total_grey_mtr
