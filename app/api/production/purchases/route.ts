...
import { PurchaseFormData } from "@/types/production";
...
export async function POST(request: NextRequest) {
  try {
    const supabase = await createClient();
    const body: PurchaseFormData = await request.json();

    // Validate required fields
    const requiredFields = ["purchase_date", "purchase_no", "material_type", "total_meters"];
    const missingFields = requiredFields.filter((field) => !body[field as keyof PurchaseFormData]);
