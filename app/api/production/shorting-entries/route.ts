...
export async function GET(request: Request) {
  try {
    ...
    // Apply filters
    if (search) {
      query = query.or(`entry_no.ilike.%${search}%,material_type.ilike.%${search}%,batch_number.ilike.%${search}%`);
    }
...
export async function POST(request: Request) {
  try {
    ...
    if (!entry_date || !entry_no || !material_type || !total_pieces) {
      return NextResponse.json(
        { error: "Missing required fields: entry_date, entry_no, material_type, total_pieces" },
        { status: 400 }
      );
    }
    ...
    const cleanedData = {
      ...
      damaged_pieces: body.damaged_pieces ? parseInt(String(body.damaged_pieces)) : 0,
      rejected_pieces: body.rejected_pieces ? parseInt(String(body.rejected_pieces)) : 0,
