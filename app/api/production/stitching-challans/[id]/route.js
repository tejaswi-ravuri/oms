...
export async function PUT(request, context) {
  try {
    ...
    if (body.batch_numbers !== undefined)
      cleanedData.batch_numbers = body.batch_numbers;
    if (body.product_name !== undefined)
      cleanedData.product_name = body.product_name || null;
    if (body.product_sku !== undefined)
      cleanedData.product_sku = body.product_sku || null;

    // Handle numeric fields properly
    if (body.quantity_sent !== undefined) {
      cleanedData.quantity_sent = body.quantity_sent
        ? parseInt(String(body.quantity_sent))
        : 0;
    }
    if (body.quantity_received !== undefined) {
      cleanedData.quantity_received = body.quantity_received
        ? parseInt(String(body.quantity_received))
        : 0;
    }
    if (body.rate_per_piece !== undefined) {
      cleanedData.rate_per_piece = body.rate_per_piece
        ? parseFloat(String(body.rate_per_piece))
        : null;
    }
    ...
    if (body.size_breakdown !== undefined)
      cleanedData.size_breakdown = body.size_breakdown || null;
